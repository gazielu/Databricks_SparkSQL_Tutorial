...
SELECT NULL as mergeKey, updates_source.*
  ...
  WHERE customers_dim.current = 1 AND customers_dim.address <> updates_source.address  --- whats happend if we need more then 1 to update (check  some)
...
select * from customers_dim order by customerid,current desc--where current=false
